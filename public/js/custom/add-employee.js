function addForm() {
  ...
  .has-error {
    ...
    <h6 class="h3 mb-0 text-gray-800 head"><i class="fas fa-pencil-alt"></i> Add New Employee</h6>
...
$(document).ready(function() {
    ...
    $(document).on('click', '#btn_basic_info', function() {
        ...
        $("#loader").show();
        validateInputs(`${$('#nyanzi').val()}/validate-basic`, data, 'list_basic_info', 'basic_info', 'list_address_ids', 'address_ids', basic);
        $("#loader").hide();
    ...
    $(document).on('click', '#btn_address_ids', function() {
        ...
        let data = new FormData(form_data);
        $("#loader").show();
        validateInputs(`${$('#nyanzi').val()}/validate-address`, data, 'list_address_ids', 'address_ids', 'list_emplmnt_work_edu', 'emplmnt_work_edu', address_id);
        $("#loader").hide();
    });
    ...
    $(document).on('click', '#btn_emplmnt_work_edu', function() {
        ...
        let data = new FormData(form_data);
        $("#loader").show();
        validateInputs(`${$('#nyanzi').val()}/validate-employmnet`, data, 'list_emplmnt_work_edu', 'emplmnt_work_edu', 'list_bank_details', 'bank_details', employment);
        $("#loader").hide();
    });
    ...
    $(document).on('click', '#btn_bank_details', function() {
        ...
        let data = new FormData(form_data);
        $("#loader").show();
        validateInputs(`${$('#nyanzi').val()}/validate-bank`, data, 'list_bank_details', 'bank_details', 'list_spouse_nofk', 'spouse_nofk', bank);
        $("#loader").hide();
    ...
    $(document).on('click', '#btn_spouse_nofk', function() {
        ...
        let data = new FormData(form_data);
        $("#loader").show();
        validateInputs(`${$('#nyanzi').val()}/validate-spouseNoK`, data, 'list_spouse_nofk', 'spouse_nofk', 'list_depend_ssnit_benefit', 'depend_ssnit_benefit', spouse_nofk);
        $("#loader").hide();
    ...
    $(document).on('click', '#btn_depend_ssnit_benefit', function() {
        ...
        $.ajax({
            type: 'POST',
            enctype: 'multipart/form-data',
            url: `${$('#nyanzi').val()}/validate-depdSBen`,
            data: data,
            processData: false,
            contentType: false,
            cache: false,
    ...
    $(document).on('click', '.editEmployee', function() {
        ...
        // if($('#ashead').val().split('(As')){
        // alert('Sorry this employee is th')
        // }
        // $("#loader").show();
        ...
        $.ajax({
            type: 'GET',
            url: `${$('#nyanzi').val()}/getEmployee/${$(this).attr('id')}`,
            // processData: false,
            ...
            success: function(response) {
                ...
                if (response.employee[0].reporttos[0].immediateSupEmplyId) {
                    // console.log('response.employee[0].departmentId => ', response.employee[0].departmentId);
                    // getEmpUnderDept('immediatesuper', response.employee[0].departmentId, response.employee[0].reporttos[0].immediateSupEmplyId, response.employee[4].name)
                ...
                } else { getEmployees('immediatesuper', response.employee[0].departmentId, '', '') } //getEmployees('immediatesuper', '', '', ''); }
    ...
    $(document).on('change', '#dept', function() {
        // alert('All set !!');
        // console.log('Dept => ', $('#dept').val());
        if ($('#dept').val() == '') {
            $('#immediatesuper').html($("<option></option>").attr("value", "").text("Select immediate supervisor"));
            return false;
        }
        getEmployees('immediatesuper', $('#dept').val(), '', '')
    })
})